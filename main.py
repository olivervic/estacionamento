...
class MeuAplicativo(App):
    permissao = None  # Armazena a permissão do usuário logado
    usuario_id = None  # Inicialize o ID do usuário como None
    ...
    def carregar_dados(self):
        ...
        try:
            ...
            if self.permissao == "visualizar_tudo":
                ...
                # Filtrar pelos carros do dia atual
                dia_ingles = datetime.now().strftime("%a")  # Exemplo: "Mon", "Tue"
                dia_atual = DIA_SEMANA_TRADUZIDO.get(dia_ingles,
                                                     "").lower()  # Traduz para português e converte para minúscula
                print(f"Carregando carros do dia atual: {dia_atual}")
    ...
    def atualizar_dados(self, dt):
        ...
        if self.usuario_id is None or self.permissao is None:
            print("Usuário não está logado. Ignorando atualização.")
            return
    ...
    def adicionar_linha_visualizacao(self, main_box, vaga, carro, placa, responsavel, horario_entrada, horario_saida,
        ...
        # Determina o status
        if horario_entrada_str == "00:00:00" and horario_saida_str == "00:00:00":
            status = "LIBERADO"
        else:
            status = calcular_tempo_restante(horario_saida_str)
        ...
        if status == "Expirado":
            label_status.color = (1, 0, 0, 1)  # Vermelho
        elif status == "LIBERADO":
            label_status.color = (0, 1, 0, 1)  # Verde
    ...
    def salvar_dados(self, vaga, carro, placa, responsavel, entrada, saida, dia):
        ...
        print(
            f"Salvando: vaga={vaga}, carro={carro}, placa={placa}, responsavel={responsavel}, entrada={entrada}, saida={saida}, dia={dia}"
        )

        if not self.validar_horario(entrada):
            print(f"Erro: Horário de entrada inválido: {entrada}")
            return

        if not self.validar_horario(saida):
            print(f"Erro: Horário de saída inválido: {saida}")
            return
    ...
    def validar_horario(self, horario):
        ...
        try:
            ...
            print(f"Erro: Horário inválido: {horario}")
            return False
